...
RSpec.describe 'invoices show' do
  ...
  it "see the total revenue for my merchant from this invoice, including discounts" do 
    ...
    ii_14 = InvoiceItem.create!(invoice_id: invoice_1.id, item_id: item_9.id, quantity: 12, unit_price: 1000, status: 2)
    ...
    expect(page).to have_content("Total Revenue (without discounts): $#{invoice_1.total_merchant_revenue(merchant1)}") #490
    expect(page).to have_content("Revenue Including Discounts: $#{invoice_1.total_discounted_revenue(merchant1).round(2)}") # 411.75
